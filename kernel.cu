...
void sgpu_SpGEMM(const CSR &dA, const CSR &dB, int *drowIds, const vector<int> &hv, CSR &dC, int cNnz, int *temp_C_128_id, QValue *temp_C_128_val, int *temp_C_256_id, QValue *temp_C_256_val, int *temp_C_512_id, QValue *temp_C_512_val, int *temp_C_1024_id, QValue *temp_C_1024_val) {
  int m = dA.rows;
  int n = dB.cols;
...
*/
  ...
  if(hv[9] - hv[8] > 0) {
  HANDLE_ERROR(cudaMalloc((void**)&temp_C_128_val,128 * (hv[9] - hv[8]) * sizeof(QValue))); 
  HANDLE_ERROR(cudaMalloc((void**)&temp_C_128_id,128 * (hv[9] - hv[8]) * sizeof(int)));
  }
  ...
  if(hv[12] - hv[11] > 0) {
  HANDLE_ERROR(cudaMalloc((void**)&temp_C_1024_val,1024 * (hv[12]-hv[11]) * sizeof(QValue)));
  HANDLE_ERROR(cudaMalloc((void**)&temp_C_1024_id,1024 * (hv[12]-hv[11]) * sizeof(int))); 
  }

//  cudaMemGetInfo(&free, &tot);
 // printf("free %d, total %d \n" , free/(1024*1024),tot/(1024*1024));
  

  // compute the structure of IC -- in gnnz.cuh
  gpu_compute_IC(dA, dB, drowIds, hv, dC, temp_C_128_id, temp_C_128_val, temp_C_256_id, temp_C_256_val, temp_C_512_id, temp_C_512_val, temp_C_1024_id, temp_C_1024_val);
